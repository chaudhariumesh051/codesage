...
DROP POLICY IF EXISTS "Users can manage own submissions" ON code_submissions;
DROP POLICY IF EXISTS "Users can manage own analyses" ON code_analyses;
DROP POLICY IF EXISTS "Users can manage own solutions" ON problem_solutions;
DROP POLICY IF EXISTS "Users can manage own chat history" ON chat_history;
DROP POLICY IF EXISTS "Admins can view all user profiles" ON user_profiles;
DROP POLICY IF EXISTS "Admins can view all code submissions" ON code_submissions;
DROP POLICY IF EXISTS "Admins can view all code analyses" ON code_analyses;
DROP POLICY IF EXISTS "Admins can view all problem solutions" ON problem_solutions;
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'update_code_submissions_updated_at'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'update_problem_solutions_updated_at'
  ) THEN
